 ...
 * QuizNudgeBanner
 *
 * Shown above the answer options in the quiz screen after the user has
 * answered at least one question. Shows remaining free questions as pip dots.
 *
 * Behaviour:
 *  - Hidden when user is premium or in dev mode
 *  - Hidden when 0 questions answered
 *  - Amber when questions remain
...
interface QuizNudgeBannerProps {
  questionsAnswered: number;
  isPremium: boolean;
  isDevMode: boolean;
}

export function QuizNudgeBanner({ questionsAnswered, isPremium, isDevMode }: QuizNudgeBannerProps) {
  const colors = useColors();
  const router = useRouter();
  const [dismissed, setDismissed] = useState(false);

  if (isPremium || isDevMode || questionsAnswered === 0 || dismissed) return null;

  const limit = FREE_LIMITS.quizQuestionsPerDay;
  const remaining = Math.max(0, limit - questionsAnswered);
  ...
  return (
    <TouchableOpacity
      onPress={handlePress}
      activeOpacity={0.85}
      accessibilityLabel="Upgrade to premium for unlimited quiz questions"
        ...
        <View style={styles.pipRow}>
          {Array.from({ length: limit }).map((_, i) => (
            <View
              ...
              style={[
                styles.pip,
                { backgroundColor: i < questionsAnswered ? accentColor : `${accentColor}28` },
              ]}
            />
          ))}
        </View>
