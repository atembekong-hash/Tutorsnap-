...
## Round 65b: Crash Fix Verification + All Suggested Improvements
- [x] Write 16 unit tests verifying SubjectRing crash fix and getDailyGoalPercent fix (73/73 passing)
- [x] Audit entire codebase for other createAnimatedComponent+SVG crash patterns — none found
- [x] Integrate Sentry crash reporting: lib/sentry.ts, initSentry() in _layout.tsx, setSentryUser in AuthGuard
- [x] Add @sentry/react-native/expo plugin to app.config.ts for source map upload on EAS builds
...
## Round 67: Complete Sentry Integration + EAS Build
- [ ] Audit all critical try/catch blocks and async operations
- [ ] Wire captureError() into server routes (academic.solve, logFeedback, auth)
- [ ] Wire captureError() into screen catch blocks (progress, chat, practice, solution)
- [ ] Wire captureError() into async operations (_layout.tsx, lib/*.ts)
- [ ] Add global JS ErrorBoundary component wrapping app root
- [ ] Wrap RootLayout with Sentry.wrap() for native crash capture
- [ ] TypeScript: 0 errors, Tests: all passing
- [ ] Bump version to 1.5.1, versionCode 20
- [ ] Save checkpoint
- [ ] Trigger EAS APK build via CLI

## Round 67: Complete Sentry Integration (v1.5.1)
- [x] Audit all try/catch blocks across 80+ files (448 catch blocks mapped)
- [x] Install @sentry/node for server-side error reporting
- [x] Create server/_core/sentry-server.ts with initSentryServer(), captureServerError(), addServerBreadcrumb()
- [x] Wire initSentryServer() into server/_core/index.ts (earliest possible startup)
- [x] Wire captureServerError() into academic.solve, academic.solveFromImage, academic.generatePractice
- [x] Wire captureServerError() into aire.logFeedback, aire.getSubjectCalibrations, aire.getThresholds
- [x] Wire captureError() into auth-screen.tsx (Google, Apple, Email OTP, Verify OTP)
- [x] Wire captureError() into progress.tsx loadProgress() critical catch
- [x] Wire addBreadcrumb() into _layout.tsx non-critical catch blocks
- [x] Add Sentry captureError to existing ErrorBoundary.componentDidCatch
- [x] Wrap app root with ErrorBoundary in _layout.tsx
- [x] TypeScript: 0 errors
- [x] Tests: 77/77 passing
- [x] Version bumped to 1.5.1 (versionCode 20, buildNumber 20)

## Round 68: User Test + Three Improvements
- [ ] Real-user test: all key flows (solve, practice, progress, chat, settings, Sentry error boundary)
- [ ] Add SENTRY_ORG and SENTRY_PROJECT secrets + wire into eas.json for source map uploads
- [ ] AIRE Stage 5: calibration decay — reset multiplier to 1.0 after 30 days inactivity (DB + server + analytics UI)
- [ ] TypeScript: 0 errors, tests: all passing
- [ ] Version bumped and checkpoint saved
