...
// ─── AIRE Stage 5: Calibration Decay ──────────────────────────────────────────

describe("AIRE Stage 5 - Calibration Decay (30-day window)", () => {
  const DECAY_DAYS = 30;

  function daysUntilDecay(lastFeedbackAt: Date): number {
    const daysSince = (Date.now() - lastFeedbackAt.getTime()) / (1000 * 60 * 60 * 24);
    return Math.max(0, Math.ceil(DECAY_DAYS - daysSince));
  }

  function isDecayed(lastFeedbackAt: Date): boolean {
    const daysSince = (Date.now() - lastFeedbackAt.getTime()) / (1000 * 60 * 60 * 24);
    return daysSince > DECAY_DAYS;
  }

  it("should not be decayed when lastFeedbackAt is today", () => {
    const now = new Date();
    expect(isDecayed(now)).toBe(false);
    expect(daysUntilDecay(now)).toBe(30);
  });

  it("should not be decayed when lastFeedbackAt is 15 days ago", () => {
    const fifteenDaysAgo = new Date(Date.now() - 15 * 24 * 60 * 60 * 1000);
    expect(isDecayed(fifteenDaysAgo)).toBe(false);
    expect(daysUntilDecay(fifteenDaysAgo)).toBe(15);
  });

  it("should be decayed when lastFeedbackAt is 31 days ago", () => {
    const thirtyOneDaysAgo = new Date(Date.now() - 31 * 24 * 60 * 60 * 1000);
    expect(isDecayed(thirtyOneDaysAgo)).toBe(true);
    expect(daysUntilDecay(thirtyOneDaysAgo)).toBe(0);
  });

  it("should return 0 daysUntilDecay when already decayed (60 days ago)", () => {
    const sixtyDaysAgo = new Date(Date.now() - 60 * 24 * 60 * 60 * 1000);
    expect(isDecayed(sixtyDaysAgo)).toBe(true);
    expect(daysUntilDecay(sixtyDaysAgo)).toBe(0);
  });

  it("should show 1 day remaining when lastFeedbackAt is 29 days ago", () => {
    const twentyNineDaysAgo = new Date(Date.now() - 29 * 24 * 60 * 60 * 1000);
    expect(isDecayed(twentyNineDaysAgo)).toBe(false);
    expect(daysUntilDecay(twentyNineDaysAgo)).toBe(1);
  });
});
