...
var users, referralCodes, fraudAlerts, redemptionHistory, otpCodes, otpAudit, schedulerLocks, aireFeedback, aireSubjectCalibration, solveHistory, chatSessions, userProgress, userBookmarks, userNotes, subscriptions;
var init_schema = __esm({
  "drizzle/schema.ts"() {
    ...
    redemptionHistory = mysqlTable("redemption_history", {
      id: int("id").autoincrement().primaryKey(),
      userId: int("userId").references(() => users.id, { onDelete: "cascade" }),
      codeId: int("codeId"),
    ...
    otpCodes = mysqlTable(
      ...
      (t2) => ({
        idxEmailPurpose: index("idx_otp_email_purpose").on(t2.email, t2.purpose, t2.createdAt),
        idxExpires: index("idx_otp_expires").on(t2.expiresAt)
      })
    );
    otpAudit = mysqlTable(
      "otp_audit",
      ...
      (t2) => ({
        idxAuditEmailCreated: index("idx_audit_email_created").on(t2.email, t2.createdAt),
        idxAuditIpCreated: index("idx_audit_ip_created").on(t2.ipAddress, t2.createdAt)
      })
    );
    schedulerLocks = mysqlTable("scheduler_locks", {
    ...
    aireFeedback = mysqlTable(
      ...
      (t2) => [index("aire_feedback_userId_idx").on(t2.userId)]
    );
    aireSubjectCalibration = mysqlTable(
      ...
      (t2) => [
        index("aire_calib_userId_subject_idx").on(t2.userId, t2.subject)
      ]
    );
    solveHistory = mysqlTable(
      "solve_history",
      {
        id: int("id").autoincrement().primaryKey(),
        userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
      ...
      (t2) => [index("solve_history_userId_idx").on(t2.userId, t2.solvedAt)]
    );
    chatSessions = mysqlTable(
      ...
      (t2) => [
        index("chat_sessions_userId_idx").on(t2.userId),
        index("chat_sessions_sessionId_idx").on(t2.userId, t2.sessionId)
      ]
    );
    ...
    userBookmarks = mysqlTable(
      ...
      (t2) => [
        index("user_bookmarks_userId_idx").on(t2.userId),
        index("user_bookmarks_bookmarkId_idx").on(t2.userId, t2.bookmarkId)
      ]
    );
    userNotes = mysqlTable(
      "user_notes",
      {
        id: int("id").autoincrement().primaryKey(),
        userId: int("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
      ...
      (t2) => [
        index("user_notes_userId_idx").on(t2.userId),
        index("user_notes_noteId_idx").on(t2.userId, t2.noteId)
      ]
    );
    subscriptions = mysqlTable(
      ...
      (t2) => [
        index("idx_subscriptions_userId").on(t2.userId),
        index("idx_subscriptions_rcUserId").on(t2.revenueCatUserId)
      ]
    );
  }
});
...
async function getAppearanceSettings(userId) {
  const db2 = await getDb();
  if (!db2) return null;
  const result = await db2.select({ appearanceSettings: users.appearanceSettings }).from(users).where(eq(users.id, userId)).limit(1);
  return result.length > 0 ? result[0].appearanceSettings ?? null : null;
}
async function saveAppearanceSettings(userId, settings) {
  const db2 = await getDb();
  if (!db2) return;
  await db2.update(users).set({ appearanceSettings: settings }).where(eq(users.id, userId));
}
var _db, db;
var init_db = __esm({
  "server/db.ts"() {
...
var t, loggingMiddleware, router, publicProcedure, requireUser, protectedProcedure, adminProcedure;
var init_trpc = __esm({
  "server/_core/trpc.ts"() {
    "use strict";
    init_const();
...
import { eq as eq5, and as and3, lt, gt as gt2, sql } from "drizzle-orm";
...
async function issueOtp(email, purpose, clientIp) {
  ...
  const emailSendCount = await db2.select({ count: sql`COUNT(*)` }).from(otpAudit).where(and3(eq5(otpAudit.email, email), gt2(otpAudit.createdAt, windowStart)));
  ...
  if (clientIp !== "unknown") {
    const ipSendCount = await db2.select({ count: sql`COUNT(*)` }).from(otpAudit).where(and3(eq5(otpAudit.ipAddress, clientIp), gt2(otpAudit.createdAt, windowStart)));
  ...
  const recent = await db2.select({ createdAt: otpAudit.createdAt }).from(otpAudit).where(
    and3(
      eq5(otpAudit.email, email),
      eq5(otpAudit.purpose, purpose),
      eq5(otpAudit.outcome, "sent"),
      gt2(otpAudit.createdAt, new Date(now.getTime() - RESEND_COOLDOWN_MS))
    )
  ).limit(1);
  if (recent.length > 0) {
  ...
  await db2.delete(otpCodes).where(and3(eq5(otpCodes.email, email), eq5(otpCodes.purpose, purpose)));
...
async function verifyOtpCode(email, code, purpose) {
  ...
  return await db2.transaction(
    async (tx) => {
      const rows = await tx.select().from(otpCodes).where(and3(eq5(otpCodes.email, email), eq5(otpCodes.purpose, purpose))).limit(1);
      const entry = rows[0];
      if (!entry) {
        return { ok: false, error: "No code found for this email. Please request a new one." };
      }
      if (/* @__PURE__ */ new Date() > new Date(entry.expiresAt)) {
        await tx.delete(otpCodes).where(eq5(otpCodes.id, entry.id));
      ...
      const incrementResult = await tx.update(otpCodes).set({ attempts: sql`${otpCodes.attempts} + 1` }).where(
        and3(
          eq5(otpCodes.id, entry.id),
          lt(otpCodes.attempts, MAX_ATTEMPTS),
          gt2(otpCodes.expiresAt, /* @__PURE__ */ new Date())
        )
      );
      ...
      const deleteResult = await tx.delete(otpCodes).where(and3(eq5(otpCodes.id, entry.id), eq5(otpCodes.hashedCode, entry.hashedCode)));
...
async function runCleanupIfLockAcquired() {
  ...
  try {
    ...
    const lockRows = await db2.select().from((init_schema(), __toCommonJS(schema_exports)).schedulerLocks).where(eq5((init_schema(), __toCommonJS(schema_exports)).schedulerLocks.jobName, "otp-cleanup")).limit(1);
    if (!lockRows[0] || lockRows[0].instanceId !== INSTANCE_ID) {
      return;
    }
    const cutoff = /* @__PURE__ */ new Date();
    const result = await db2.delete(otpCodes).where(lt(otpCodes.expiresAt, cutoff));
    const otpDeleted = result[0]?.affectedRows ?? 0;
    const auditCutoff = new Date(now.getTime() - 24 * 60 * 60 * 1e3);
    const auditResult = await db2.delete(otpAudit).where(lt(otpAudit.createdAt, auditCutoff));
    const auditDeleted = auditResult[0]?.affectedRows ?? 0;
  } catch (err) {
    console.warn("[OTP Cleanup] Scheduler error (non-fatal):", err?.message ?? err);
  }
}
var OTP_TTL_MS, RESEND_COOLDOWN_MS, MAX_ATTEMPTS, RATE_LIMIT_WINDOW_MS, MAX_SENDS_PER_EMAIL, MAX_SENDS_PER_IP, DEFAULT_TRUSTED_CIDRS, emailAuthRouter, CLEANUP_INTERVAL_MS, LOCK_TTL_MS, INSTANCE_ID;
var init_email_auth = __esm({
  "server/routers/email-auth.ts"() {
    ...
    emailAuthRouter = router({
       ...
       */
      sendOtp: publicProcedure.input(z4.object({ email: z4.string().email() })).mutation(async ({ input, ctx }) => {
      ...
      verifyOtp: publicProcedure.input(
        ...
        try {
          ...
          const existingRows = await db2.select().from(users).where(eq5(users.openId, openId)).limit(1);
          ...
          if (!user) {
            ...
            const newRows = await db2.select().from(users).where(eq5(users.openId, openId)).limit(1);
            user = newRows[0];
          } else {
            await db2.update(users).set({ lastSignedIn: /* @__PURE__ */ new Date() }).where(eq5(users.openId, openId));
          }
       ...
       */
      sendChangeEmailOtp: protectedProcedure.input(z4.object({ newEmail: z4.string().email() })).mutation(async ({ input, ctx }) => {
        const newEmail = input.newEmail.toLowerCase().trim();
        try {
          const db2 = await getDb();
          if (db2) {
            const existing = await db2.select({ id: users.id }).from(users).where(eq5(users.email, newEmail)).limit(1);
       ...
       */
      verifyChangeEmail: protectedProcedure.input(z4.object({ newEmail: z4.string().email(), code: z4.string().length(6) })).mutation(async ({ ctx, input }) => {
        const newEmail = input.newEmail.toLowerCase().trim();
        ...
        try {
          const db2 = await getDb();
          if (!db2) return { success: false, error: "Database unavailable" };
          await db2.update(users).set({ email: newEmail, updatedAt: /* @__PURE__ */ new Date() }).where(eq5(users.id, ctx.user.id));
    ...
    INSTANCE_ID = `${hostname()}-${process.pid}`;
  }
});
...
function getMathJax() {
  const mj = __require("mathjax-node");
...
import { eq as eq6, desc, and as and4, sql as sql2 } from "drizzle-orm";
...
async function checkRapidRedemption(userId) {
  ...
  const recentRedemptions = await db2.select().from(redemptionHistory).where(
    and(
      eq2(redemptionHistory.userId, userId),
      gte(redemptionHistory.createdAt, fiveMinutesAgo),
      eq2(redemptionHistory.success, true)
    )
  );
...
async function checkHighValueCodeAbuse(code, userId) {
  ...
  const recentAttempts = await db2.select().from(redemptionHistory).where(
    and(
      eq2(redemptionHistory.code, code),
...
var referralRouter = router({
   ...
   */
  validateCode: publicProcedure.input(z2.object({ code: z2.string().min(5).max(20), userId: z2.number(), ipAddress: z2.string().optional(), deviceId: z2.string().optional() })).mutation(async ({ input }) => {
    try {
      const db2 = await getDb();
      if (!db2) {
        return {
      ...
      const codeData = await db2.select().from(referralCodes).where(eq3(referralCodes.code, codeUpper)).limit(1);
      ...
      await db2.update(referralCodes).set({ uses: code.uses + 1 }).where(eq3(referralCodes.id, code.id));
      await logRedemptionAttempt({
        userId: input.userId,
        code: codeUpper,
        ipAddress: input.ipAddress,
   ...
   */
  generateCode: publicProcedure.input(z2.object({ userId: z2.number() })).mutation(async ({ input }) => {
    try {
      const db2 = await getDb();
      if (!db2) {
        return {
   ...
   */
  getCodeStats: publicProcedure.input(z2.object({ code: z2.string() })).query(async ({ input }) => {
    try {
      const db2 = await getDb();
      if (!db2) {
        return {
      ...
      const codeData = await db2.select().from(referralCodes).where(eq3(referralCodes.code, input.code.toUpperCase())).limit(1);
      if (codeData.length === 0) {
        return {
          found: false,
          uses: 0,
   ...
   */
  getUserCodes: publicProcedure.input(z2.object({ userId: z2.number() })).query(async ({ input }) => {
    try {
      const db2 = await getDb();
      if (!db2) {
        return {
      ...
      const codes = await db2.select().from(referralCodes).where(eq3(referralCodes.userId, input.userId));
      return {
        success: true,
        codes: codes.map((c) => ({
          code: c.code,
...
var oauthRouter = router({
  ...
  validate: publicProcedure.input(
    z3.object({
      provider: z3.enum(["google", "apple"]),
      idToken: z3.string().min(10),
    ...
    try {
      ...
      let existingUser = await db2.select().from(users).where(eq4(users.openId, openId)).limit(1);
      let user;
      if (existingUser.length > 0) {
        ...
        const newUser = await db2.select().from(users).where(eq4(users.openId, openId)).limit(1);
        user = newUser[0];
      }
      return {
        success: true,
   ...
   */
  revoke: publicProcedure.input(z3.object({ provider: z3.enum(["google", "apple"]), token: z3.string().optional() })).mutation(async ({ input }) => {
  ...
  getProfile: publicProcedure.input(z3.object({ userId: z3.number() })).query(async ({ input }) => {
    try {
      ...
      const user = await db2.select().from(users).where(eq4(users.id, input.userId)).limit(1);
      if (user.length === 0) {
        return {
          success: false,
  ...
  updateProfile: publicProcedure.input(
    ...
    try {
      ...
      await db2.update(users).set(updates).where(eq4(users.id, input.userId));
      return {
        success: true,
...
function extractLLMContent(result) {
  if (result?.error) {
    ...
    throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR", message: `AI service error: ${msg}` });
  }
...
var academicRouter = router({
  solve: publicProcedure.input(z5.object({
    problem: z5.string().min(1),
    subject: z5.string().default("other"),
    gradeLevel: z5.string().nullable().optional()
    ...
    try {
      ...
      if (ctx.user) {
        try {
          const db2 = await getDb();
          if (db2) {
            const cacheRows = await db2.select({ multiplier: aireSubjectCalibration.multiplier }).from(aireSubjectCalibration).where(and4(
              eq6(aireSubjectCalibration.userId, ctx.user.id),
              eq6(aireSubjectCalibration.subject, input.subject)
            )).limit(1);
      ...
      throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR", message: err instanceof Error ? err.message : "Failed to solve problem. Please try again." });
    }
  }),
  solveExplanation: publicProcedure.input(z5.object({
    problem: z5.string().min(1, "problem is required"),
    correctAnswer: z5.string().min(1, "correctAnswer is required"),
    selectedAnswer: z5.string().min(1, "selectedAnswer is required"),
...
Respond ONLY with this JSON (no extra text):
{
  ...
  solveFromImage: publicProcedure.input(z5.object({
    ...
    try {
      ...
      throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR", message: err instanceof Error ? err.message : "Failed to process image. Please try again." });
    }
  }),
  generatePractice: publicProcedure.input(z5.object({
    subject: z5.string(),
    ...
    try {
      ...
      try {
        ...
        throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR", message: "AI returned invalid JSON. Please try again." });
      }
    }
  ...
  generateQuiz: publicProcedure.input(z5.object({
    subject: z5.string(),
    difficulty: z5.enum(["easy", "medium", "hard"]),
    count: z5.number().min(3).max(10).default(5),
    gradeLevel: z5.string().optional()
  })).mutation(async ({ input }) => {
...
{"questions":[{"id":"q1","problem":"<question>","options":{"A":"<a>","B":"<b>","C":"<c>","D":"<d>"},"correctAnswer":"A","explanation":"<1 sentence>"}]}`;
    ...
    try {
      ...
      throw new TRPCError3({ code: "BAD_REQUEST", message: "Invalid JSON in AI response" });
    }
    return parsed.questions ?? [];
  }),
  studyTip: publicProcedure.input(z5.object({
    subject: z5.string(),
    gradeLevel: z5.string().optional()
  })).mutation(async ({ input }) => {
  ...
  chat: publicProcedure.input(z5.object({
    ...
    gradeLevel: z5.string().optional(),
    detailedMode: z5.boolean().optional()
...
ADAPT YOUR RESPONSE to this student's level: ${GRADE_LEVEL_DESCRIPTIONS2[input.gradeLevel]}` : "";
  ...
  suggestFollowUps: publicProcedure.input(z5.object({
    aiResponse: z5.string(),
    subject: z5.string().optional()
  })).mutation(async ({ input }) => {
...
{"chips": ["Question 1", "Question 2", "Question 3"]}`;
  ...
  explainDifferently: publicProcedure.input(z5.object({
    problem: z5.string().min(1),
    answer: z5.string(),
    subject: z5.string().default("other"),
    gradeLevel: z5.string().optional(),
    style: z5.enum(["analogy", "step-by-step", "visual"]).default("analogy")
  })).mutation(async ({ input }) => {
...
Now re-explain this using the ${input.style} style.`;
  ...
  generateSimilar: publicProcedure.input(z5.object({
    problem: z5.string(),
    subject: z5.string(),
    difficulty: z5.enum(["easy", "medium", "hard"]).default("medium"),
    count: z5.number().min(1).max(5).default(3),
    gradeLevel: z5.string().optional()
  })).mutation(async ({ input }) => {
...
{"problems":[{"id":"p1","problem":"<problem>","hint":"<1-sentence hint>"}]}`;
  ...
  generateStudyBlocks: publicProcedure.input(z5.object({
    problem: z5.string(),
    answer: z5.string(),
    steps: z5.array(z5.object({
      stepNumber: z5.number(),
      title: z5.string(),
      explanation: z5.string(),
      expression: z5.string().optional()
    })).optional(),
    conceptExplained: z5.string().optional(),
    tips: z5.array(z5.string()).optional(),
    subject: z5.string(),
    gradeLevel: z5.string().optional()
  })).mutation(async ({ input }) => {
...
var voiceRouter = router({
  ...
  getUploadUrl: publicProcedure.input(z5.object({ filename: z5.string(), contentType: z5.string() })).mutation(async ({ input }) => {
...
var userRouter = router({
  ...
  saveAppearanceSettings: protectedProcedure.input(z5.object({ settings: z5.string().max(65535) })).mutation(async ({ ctx, input }) => {
...
async function computeSubjectMultiplier(db2, userId, subject) {
  if (!db2) return 1;
  const rows = await db2.select({ rating: aireFeedback.rating }).from(aireFeedback).where(and4(eq6(aireFeedback.userId, userId), eq6(aireFeedback.subject, subject))).orderBy(desc(aireFeedback.createdAt)).limit(20);
...
async function refreshSubjectCalibration(db2, userId, subject) {
  ...
  const rows = await db2.select({ id: aireSubjectCalibration.id }).from(aireSubjectCalibration).where(and4(eq6(aireSubjectCalibration.userId, userId), eq6(aireSubjectCalibration.subject, subject))).limit(1);
  const countRows = await db2.select({ cnt: sql2`count(*)` }).from(aireFeedback).where(and4(eq6(aireFeedback.userId, userId), eq6(aireFeedback.subject, subject)));
  const sampleCount = Number(countRows[0]?.cnt ?? 0);
  if (rows.length > 0) {
    await db2.update(aireSubjectCalibration).set({ multiplier: String(multiplier), sampleCount }).where(and4(eq6(aireSubjectCalibration.userId, userId), eq6(aireSubjectCalibration.subject, subject)));
  } else {
...
var aireRouter = router({
   ...
   */
  logFeedback: publicProcedure.input(z5.object({
    difficulty: z5.number().int().min(1).max(5),
    subject: z5.string().default("other"),
    steps: z5.number().int().min(0).default(1),
    rating: z5.number().int().min(-1).max(1)
  })).mutation(async ({ ctx, input }) => {
    ...
    try {
      ...
      if (userId) {
        const rows = await db2.select({ id: aireFeedback.id }).from(aireFeedback).where(eq6(aireFeedback.userId, userId)).orderBy(desc(aireFeedback.createdAt)).limit(30);
        if (rows.length > 20) {
          const idsToDelete = rows.slice(20).map((r) => r.id);
          for (const id of idsToDelete) {
            await db2.delete(aireFeedback).where(eq6(aireFeedback.id, id));
          }
        }
        try {
  ...
  getSubjectCalibrations: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      }).from(aireSubjectCalibration).where(eq6(aireSubjectCalibration.userId, ctx.user.id)).orderBy(desc(aireSubjectCalibration.sampleCount));
  ...
  getThresholds: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const rows = await db2.select({ rating: aireFeedback.rating }).from(aireFeedback).where(eq6(aireFeedback.userId, userId)).orderBy(desc(aireFeedback.createdAt)).limit(10);
      if (rows.length < 3) {
...
var cloudSyncRouter = router({
  pushSolveHistory: protectedProcedure.input(z5.object({
    items: z5.array(z5.object({
      problem: z5.string(),
      answer: z5.string().optional(),
      subject: z5.string().optional(),
      solutionJson: z5.string().optional(),
  ...
  pullSolveHistory: protectedProcedure.query(async ({ ctx }) => {
    try {
      const db2 = await getDb();
      if (!db2) return { items: [] };
      const rows = await db2.select().from(solveHistory).where(eq6(solveHistory.userId, ctx.user.id)).orderBy(desc(solveHistory.solvedAt)).limit(200);
      return {
        items: rows.map((r) => ({
          problem: r.problem,
          answer: r.answer ?? "",
  ...
  pushChatSession: protectedProcedure.input(z5.object({
    sessionId: z5.string().max(64),
    title: z5.string().max(255).optional(),
    subject: z5.string().max(64).optional(),
    gradeLevel: z5.string().max(32).optional(),
    messagesJson: z5.string(),
    tags: z5.string().optional(),
    pinned: z5.boolean().optional(),
    messageCount: z5.number().int().optional(),
    sessionCreatedAt: z5.number(),
    sessionUpdatedAt: z5.number()
  })).mutation(async ({ ctx, input }) => {
    try {
      const db2 = await getDb();
      if (!db2) return { ok: false };
      const userId = ctx.user.id;
      const existing = await db2.select({ id: chatSessions.id }).from(chatSessions).where(and4(eq6(chatSessions.userId, userId), eq6(chatSessions.sessionId, input.sessionId))).limit(1);
      if (existing.length > 0) {
        ...
        }).where(and4(eq6(chatSessions.userId, userId), eq6(chatSessions.sessionId, input.sessionId)));
      } else {
  ...
  deleteChatSession: protectedProcedure.input(z5.object({ sessionId: z5.string().max(64) })).mutation(async ({ ctx, input }) => {
    try {
      const db2 = await getDb();
      if (!db2) return { ok: false };
      await db2.delete(chatSessions).where(and4(eq6(chatSessions.userId, ctx.user.id), eq6(chatSessions.sessionId, input.sessionId)));
      return { ok: true };
    } catch (err) {
  ...
  pullChatSessions: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const rows = await db2.select().from(chatSessions).where(eq6(chatSessions.userId, ctx.user.id)).orderBy(desc(chatSessions.sessionUpdatedAt)).limit(100);
      return {
        sessions: rows.map((r) => ({
  ...
  pushProgress: protectedProcedure.input(z5.object({ progressJson: z5.string() })).mutation(async ({ ctx, input }) => {
    try {
      const db2 = await getDb();
      if (!db2) return { ok: false };
      const userId = ctx.user.id;
      const existing = await db2.select({ id: userProgress.id }).from(userProgress).where(eq6(userProgress.userId, userId)).limit(1);
      if (existing.length > 0) {
        await db2.update(userProgress).set({ progressJson: input.progressJson }).where(eq6(userProgress.userId, userId));
      } else {
  ...
  pullProgress: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const rows = await db2.select({ progressJson: userProgress.progressJson }).from(userProgress).where(eq6(userProgress.userId, ctx.user.id)).limit(1);
  ...
  pushBookmarks: protectedProcedure.input(z5.object({
    bookmarks: z5.array(z5.object({
      bookmarkId: z5.string().max(64),
      itemJson: z5.string(),
      subject: z5.string().max(64).optional()
    })).max(200)
  })).mutation(async ({ ctx, input }) => {
    try {
      ...
      if (!db2) return { ok: false };
      const userId = ctx.user.id;
      await db2.delete(userBookmarks).where(eq6(userBookmarks.userId, userId));
  ...
  pullBookmarks: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const rows = await db2.select().from(userBookmarks).where(eq6(userBookmarks.userId, ctx.user.id)).orderBy(desc(userBookmarks.createdAt)).limit(200);
      return {
  ...
  pushNotes: protectedProcedure.input(z5.object({
    notes: z5.array(z5.object({
      noteId: z5.string().max(64),
      noteJson: z5.string()
    })).max(500)
  })).mutation(async ({ ctx, input }) => {
    try {
      ...
      if (!db2) return { ok: false };
      const userId = ctx.user.id;
      await db2.delete(userNotes).where(eq6(userNotes.userId, userId));
  ...
  pullNotes: protectedProcedure.query(async ({ ctx }) => {
    try {
      const db2 = await getDb();
      if (!db2) return { notes: [] };
      const rows = await db2.select().from(userNotes).where(eq6(userNotes.userId, ctx.user.id)).orderBy(desc(userNotes.updatedAt)).limit(500);
      return {
        notes: rows.map((r) => ({
          noteId: r.noteId,
          noteJson: r.noteJson
  ...
  pullAll: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const [historyRows, chatRows, progressRows, bookmarkRows, noteRows] = await Promise.all([
        db2.select().from(solveHistory).where(eq6(solveHistory.userId, userId)).orderBy(desc(solveHistory.solvedAt)).limit(200),
        db2.select().from(chatSessions).where(eq6(chatSessions.userId, userId)).orderBy(desc(chatSessions.sessionUpdatedAt)).limit(100),
        db2.select({ progressJson: userProgress.progressJson }).from(userProgress).where(eq6(userProgress.userId, userId)).limit(1),
        db2.select().from(userBookmarks).where(eq6(userBookmarks.userId, userId)).orderBy(desc(userBookmarks.createdAt)).limit(200),
        db2.select().from(userNotes).where(eq6(userNotes.userId, userId)).orderBy(desc(userNotes.updatedAt)).limit(500)
      ]);
      return {
...
var subscriptionRouter = router({
  ...
  getStatus: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const rows = await db2.select().from(subscriptions).where(eq6(subscriptions.userId, ctx.user.id)).orderBy(desc(subscriptions.updatedAt)).limit(1);
      if (rows.length === 0) {
  ...
  history: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      }).from(subscriptions).where(eq6(subscriptions.userId, ctx.user.id)).orderBy(desc(subscriptions.updatedAt)).limit(50);
      return rows.map((r) => {
